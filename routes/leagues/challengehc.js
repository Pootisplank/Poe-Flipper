...
router.get('/', async (req, res) => {
    console.log('/challengehc endpoint called')
    const dataCurrency = await getData('Currency')
    const currency = processData(dataCurrency)
    res.render('leagues/challengehc/currency', {currency:currency})
})

router.get('/currency', async (req, res) => {
    console.log('/challengehc/currency endpoint called')
    const dataCurrency = await getData('Currency')
    const currency = processData(dataCurrency)
    res.render('leagues/challengehc/currency', {currency:currency})
})
...
router.get('/fragment', async (req, res) => {
    console.log('/challengehc/fragment endpoint called')
    const dataFragment = await getData('Fragment')
    const fragment = processData(dataFragment)
    res.render('leagues/challengehc/fragment', {fragment:fragment})
})
